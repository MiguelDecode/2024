...
function createMainTweet(): TweetThreat {
  const id = crypto.randomUUID();
...
const updateTweet = (
  tweetThreat: TweetThreat,
  tweet: Tweet,
  value: Message
) => {
  let newTweet: Tweet | null = null;

  // Cycle of the tweets of a threat
  for (let i = 0; i < tweetThreat.tweets.length; i++) {
    const data = tweetThreat.tweets[i];

    if (data.id === tweet.id) newTweet = data;

    if (newTweet) newTweet.message = value;
  }
};

const renderTweet = (
  tweetView: TweetThreat,
  view: HTMLDivElement,
  tweet: Tweet,
  last: boolean
) => {
  ...
  if (last) form.appendChild(buttonAddMore);

  view.appendChild(tweetContainer);
};

function renderView(tweetView: TweetThreat) {
  ...
  if (view && view instanceof HTMLDivElement) {
    for (let i = 0; i < tweetView.tweets.length; i++) {
      renderTweet(
        ...
        view,
        tweetView.tweets[i],
        i === tweetView.tweets.length - 1 // Return true if it's the last element of the array
      );
    }
  }
}
